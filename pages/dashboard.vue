<script setup lang="ts">
  ...
  const filtered = computed(() => {
    ...
    let list = sites.value.filter(s => {
      ...
      const matchesQ = !term || name.includes(term) || id.includes(term) || domain.includes(term);
      const matchesEnv =
        envFilter.value === 'all' || String(s.env || '').toLowerCase() === envFilter.value;
    ...
    if (sortBy.value === 'az') {
      ...
      list = list.sort(
        (a, b) =>
          new Date(a.nextMaintenance || 0).getTime() - new Date(b.nextMaintenance || 0).getTime()
      );
    } else {
      list = list.sort(
        (a, b) =>
          new Date(b.nextMaintenance || 0).getTime() - new Date(a.nextMaintenance || 0).getTime()
      );
    }
    return list;
  });
  ...
  const monthsOverview = computed<MonthOverview[]>(() => {
    ...
    for (let i = -monthsToShowPast; i <= monthsToShowFuture; i++) {
      ...
      const label = new Date(Date.UTC(year, monthIdx, 1)).toLocaleString(undefined, {
        month: 'long',
        year: 'numeric',
      });
      ...
      if (maintenance.value.length) {
        const inMonth = maintenance.value.filter(it => {
          ...
          return (
            d.getUTCFullYear() === year && d.getUTCMonth() === monthIdx && it.status !== 'Completed'
          );
        });
...
<template>
  <div class="min-h-screen bg-slate-100 p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    >
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-3 space-y-4">
        <div class="flex items-center justify-between gap-4">
          <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">PLOTT Maintenance</h1>
          </div>
    ...
    <main class="max-w-7xl mx-auto space-y-6">
      ...
      <div v-if="pending && !data" class="card text-center text-slate-500">
        Loading dashboard...
      </div>
      ...
      <template v-else>
        ...
        <section v-show="tab === 'overview'" class="space-y-6">
          ...
          <div class="card space-y-3">
            ...
            <template v-if="pastIssues.length">
              ...
              >
                ...
                <div class="mt-2 space-y-1">
                  ...
                  <p v-if="!pastIssuesGrouped[status].length" class="text-xs text-slate-500 ml-1">
                    None.
                  </p>
                </div>
              </details>
            ...
            <p v-else class="text-sm text-slate-500">No outstanding past issues. 🎉</p>
          </div>
          ...
          <div class="card space-y-4">
              ...
              <div v-if="accordionOpen.preRenewal" class="accordion-content p-4 space-y-2">
                ...
                >
                  ...
                  <div class="flex gap-2 text-xs">
                    <span v-if="item.status" :class="statusClass(item.status)">
                      {{ item.status }}
                    </span>
            ...
            <div class="border border-slate-200 rounded-lg overflow-hidden">
              ...
              <div v-if="accordionOpen.midYear" class="accordion-content p-4 space-y-2">
                ...
                <p v-if="!incompleteMaintenance.midYear.length" class="text-sm text-slate-500 py-2">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="card space-y-3">
              <h2 class="font-semibold text-slate-800">Reports due this month</h2>
              <NuxtLink
              ...
              <p v-if="!thisMonth.reports.length" class="text-sm text-slate-500">No reports due.</p>
            </div>

            <div class="card space-y-3">
              <h2 class="font-semibold text-slate-800">Renewals this month</h2>
              <NuxtLink
              ...
              <p v-if="!thisMonth.renewals.length" class="text-sm text-slate-500">
        ...
        <section v-show="tab === 'months'" class="space-y-4">
          <div class="card flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex gap-2">
              <button @click="showPastMonths = !showPastMonths" class="btn-secondary">
                {{ showPastMonths ? 'Hide past months' : 'Show past months' }}
              </button>
            </div>
            <label class="inline-flex items-center gap-2 text-xs text-slate-600">
              <input type="checkbox" v-model="followupOnly" class="rounded border-slate-300" />
              ...
              <div class="space-y-4 text-sm">
                ...
                <div class="space-y-1">
                  ...
                  <div v-if="m.maintenance.length" class="space-y-1">
                    <NuxtLink
                      v-for="s in followupOnly
                        ? m.maintenance.filter(x => x.status && problemStatuses.includes(x.status))
                        : m.maintenance"
                      :key="s.id"
                      :to="`/site/${s.id}`"
                      class="month-item"
                    >
                      ...
                      <div class="flex gap-2 text-[11px] items-center">
                        <span v-if="s.status" :class="statusClass(s.status)">
                          {{ s.status }}
                        </span>
                        <span v-else class="flex gap-1">
                          <span v-if="s.preRenewal" class="chip chip-amber !px-1.5 !py-0.5">
                            Pre
                          </span>
                          <span v-if="s.midYear" class="chip chip-blue !px-1.5 !py-0.5"> Mid </span>
                          <span
                  ...
                  <p v-else class="text-xs text-slate-500 px-1.5">None outstanding</p>
                </div>
                ...
                <div class="space-y-1">
                  ...
                  <p v-else class="text-xs text-slate-500 px-1.5">None</p>
                </div>
        ...
        <section v-show="tab === 'sites'" class="space-y-6">
          <div class="card">
            <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
              <div class="md:col-span-3">
                <label for="site-search" class="filter-label"> Search Site </label>
                <input
                  id="site-search"
                  v-model="q"
                  type="search"
              ...
              <div class="md:col-span-2">
                <label for="site-sort" class="filter-label"> Sort By </label>
                <select id="site-sort" v-model="sortBy" class="filter-input">
              ...
              <div class="md:col-span-1">
                ...
                <select id="site-env" v-model="envFilter" class="filter-input">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            <div v-for="s in filtered" :key="s.id" class="card flex flex-col">
              <div class="flex items-center gap-4">
                ...
                <div class="min-w-0">
                  <h3 class="font-semibold text-slate-800 leading-tight truncate">
                    {{ s.name || s.id }}
                  </h3>
              ...
              <div class="space-y-1 text-sm">
                <div class="flex justify-between">
                  ...
                  <span class="font-medium text-slate-700" :class="envBadge(s.env)">
                    {{ s.env }}
                  </span>
                </div>
                <div class="flex justify-between">
          ...
          <div v-if="!pending && filtered.length === 0" class="card text-center text-slate-500">
        ...
        <!-- PING TEST -->
        <section class="card">
          <h2 class="font-semibold text-slate-800 mb-2">Maintenance Mode Ping Test</h2>
          ...
          <div class="mt-2 text-sm min-h-[1.25rem]">
            <p v-if="testMsg" class="text-emerald-700" v-html="testMsg" />
            <p v-if="testErr" class="text-red-600">{{ testErr }}</p>
            <div v-if="testRes" class="text-xs text-slate-500 mt-1 space-y-0.5">
              <p>
                Final URL:
              ...
              <p>
                Time:
                <code class="text-slate-700"> {{ testRes.timeMs }}ms </code>
              </p>
            </div>
          </div>
        </section>
        ...
        >
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                ...
                <div class="mt-2 space-y-4">
                  <p class="text-sm text-gray-500">
                    This will
                    <strong>delete all existing maintenance items</strong> for all sites and
                    regenerate them with end-of-month, weekday-only dates.
                  </p>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button
              ...
              :disabled="bulkRebuilding || bulkRebuildForm.confirmText !== 'REBUILD ALL SITES'"
              type="button"
